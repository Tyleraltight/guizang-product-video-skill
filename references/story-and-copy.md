...
先给观众一个主张，然后逐段证明，最后以产品和行动收尾。45–60 秒通常适合 3–5 组更新、约 10–14 个镜头；这是容量参考，不是必须填满的模板。小版本可以 20 秒，复杂功能也可以更长。
...
- 镜头停留时间按画面实际信息量估算，避免机械均分时长。纯大字或单行口号 1.5 到 2.5 秒即可切走；单个组件配一句话说明通常 3 到 4 秒；包含多栏对比、代码反汇编或复杂拓扑的镜头才留 5 到 7 秒。
- 镜头时长尽量吸附到偶数音乐小节（例如 120 BPM 下 1 个小节约 2 秒，2 个小节约 4 秒），让切点落在节拍骨架上，避免在拍子中间无谓拖延。
- 每个镜头一般安排“建立画面 → 核心动作 → 可读结果 → 切走”，不要全部元素一起飘入再静止 5 秒。
- 超过 4 秒的长镜头避免静态干等。如果一个镜头需要展示较多信息，不要在第 0 秒把全部元素同时摆出来然后静止，应当按节拍拆成两到三个阶段的微动作（比如先出标题建立语境，再触发交互动作，最后呈现结果反馈），让画面始终保持推进感。
...
从实际代码截取代表镜头，按播放顺序排成一张联系表。核对是否有画面尺度变化、是否一眼知道主次、缩小到手机宽度能否读懂解释。静帧通过只能证明布局方向；节奏、动作连贯和声音仍需看成片。
