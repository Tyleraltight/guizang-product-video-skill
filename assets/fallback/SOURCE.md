...
本目录的组件和样式提取/改编自 CodePilot，作者 op7418，© 2025–2026。精确来源路径和工作区内容哈希见 `provenance.json`。

`CardFrame` 与 `CardSurface` 保留原组件的“外层阴影、内层裁切”结构，移除了业务布局属性和 Tailwind 依赖。按钮、标签改为轻量展示组件；token 保留暖白/炭黑、圆角和柔和阴影，并调整了适合影片的字号和间距。标题字体分工由影片的 CSS 变量控制。它们是改编组件，不是原仓库文件的原样复制。
