...
把代码库交给 AI，告诉它这次更新了什么，就可以开始做一支软件更新宣传片：整理卖点、写文案、接入原组件、做动画、写配乐、配音效，最后导出视频和可继续修改的工程。
...
1. **先定范围。** 确认产品、更新范围、受众、发布平台、画幅、语言和风格。
...
**音乐用代码原创。** 工程里保留合成与编曲源码，根据本片时长、节奏和结尾重新安排。内置的 48 秒配乐代码是一个起点，制作时会针对当前影片改编。
...
| Playwright Chromium | 内置浏览器管线的静帧和逐帧渲染 |
| 产品仓库与其依赖 | 直接接入原组件及样式 |

依赖安装说明按需加载，见 [入门与依赖检查](references/onboarding.md)。无需为了使用内置浏览器管线额外购买视频服务；AI 编程工具本身及你选择的外部服务按各自规则使用。

不同组件框架的接入工作量不同。React 代码库有现成示例；需要服务端、原生运行环境或复杂上下文的组件，可能还需要展示适配层。
...
scripts/                 初始化、环境检查、合成音效、混音、交付检查
assets/starter/          可运行的起步工程
assets/fallback/         CodePilot 默认样式与展示组件
assets/audio/            原创配乐示例、11 个内置音效及来源说明
